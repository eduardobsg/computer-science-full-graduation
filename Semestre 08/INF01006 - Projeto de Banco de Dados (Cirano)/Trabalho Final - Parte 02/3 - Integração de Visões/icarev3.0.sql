...
CREATE TABLE endereco (
	...
	CONSTRAINT idendereco_pkey PRIMARY KEY (idendereco)
);


--
-- Name: pessoaEndereco; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE pessoaEndereco (
	...
	CONSTRAINT pessoaEndereco_pkey PRIMARY KEY(cdlpessoa, idendereco),
	
	CONSTRAINT fk_pessoaEndereco_cdlpessoa FOREIGN KEY (cdlpessoa)
      REFERENCES mbspessoa (cdlpessoa) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION,
	
	CONSTRAINT fk_pessoaEndereco_idendereco FOREIGN KEY (idendereco)
      REFERENCES endereco (idendereco) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION
);
...
CREATE TABLE tipoAlergia (
	...
	alergiaNome character varying(255),
	alergiaDescricao character varying(4000),
	
	CONSTRAINT tipoAlergia_pkey PRIMARY KEY (idAlergia)
);
...
CREATE TABLE pacienteAlergia (
	cdlpaciente bigint NOT NULL,
	idAlergia bigint NOT NULL,
	severidade integer, -- escala de 0 a 5, sendo 0 o mais severo
	dataIdentificacaoAlergia date, -- data em que foi constatada a alergia
	
	CONSTRAINT pacienteAlergia_pkey PRIMARY KEY (cdlpaciente, idAlergia),
	
	CONSTRAINT fk_pacienteAlergia_cdlpaciente FOREIGN KEY (cdlpaciente)
      REFERENCES tmedpaciente (cdlpaciente) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION,
      
	CONSTRAINT fk_pacienteAlergia_idAlergia FOREIGN KEY (idAlergia)
      REFERENCES tipoAlergia (idAlergia) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION
);


CREATE TABLE tipoIncapacidade (
	idIncapacidade bigint NOT NULL,
	incapacidadeNome character varying(80),
	
	CONSTRAINT tipoIncapacidade_pkey PRIMARY KEY (idIncapacidade)
);


CREATE TABLE incapacidadePaciente (
	cdlpaciente bigint NOT NULL,
	idIncapacidade bigint NOT NULL,
	descricao character varying(2000),
	
	CONSTRAINT incapacidadePaciente_pkey PRIMARY KEY (cdlpaciente, idIncapacidade),
	
	CONSTRAINT fk_incapacidadePaciente_cdlpaciente FOREIGN KEY (cdlpaciente)
      REFERENCES tmedpaciente (cdlpaciente) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION,
	
	CONSTRAINT fk_incapacidadePaciente_idIncapacidade FOREIGN KEY (idIncapacidade)
      REFERENCES tipoIncapacidade (idIncapacidade) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION
);
