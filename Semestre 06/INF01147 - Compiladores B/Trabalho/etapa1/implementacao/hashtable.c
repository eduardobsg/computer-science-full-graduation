...
Entry* add(Hashtable *hashtable, char *str, int type)
{
    ...
    new_list->type = malloc(sizeof (int));
    *(new_list->type) = type;
    new_list->next = hashtable->table[hashval];
    hashtable->table[hashval] = new_list;

    printf("\tadd: inserted %s \n", str);
    return new_list;
}
...
void print(Hashtable *hashtable)
{
    ...
    for (i = 0; i < hashtable->size; i++) {
        ...
        while (entry != NULL) {
            printf("{%03i} ", i);
            printf("Entry[%s,%i]\t", entry->text, *(entry->type));
            entry = entry->next;
            printf("\n");
        }
    }
 ...
 */
void fixEntry(char *str, int type) {
	
	if (type == 285) { // LIT_CHAR
		str[0] = str[1];
		str[1] = '\0';
		return;
	}
	else if (type == 286) { // LIT_STRING
		int size = strlen(str);
		int i;
		for(i = 0; i < size; i++) {
			str[i] = str[i+1];
