...
Entry *makeTemp()
{
    ...
    // Create a _ScArYTeMpOrArY_SEQ_NUMBER and return
    //sprintf(str, "_ScArYTeMpOrArY_%d", ++tempNum);
    sprintf(str, "_tmp_%d", ++tempNum);
    return add(hashtable, str, TK_IDENTIFIER);
}

// create a label for JUMP-like bindings
// used in IFs and other structures
Entry * makeLabel()
{
    static int nextLabel = 0;
    char buffer[128];
    
    sprintf(buffer, "_lbl_%d", ++nextLabel);
    return add(hashtable, buffer, TK_IDENTIFIER);
}
