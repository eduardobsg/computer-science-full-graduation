...
void astreePrintSingle(ASTREE *node)
{
    ...
    case ASTREE_UNARY_MINUS: fprintf(stderr, "EXP-UNARY-MINUS");
        break;
    case ASTREE_POINTER: fprintf(stderr, "EXP-UNARY-POINTER");
        break;
    case ASTREE_DEREFERENCE: fprintf(stderr, "EXP-DEREFERENCE");
        break;
...
void n2str(int level, ASTREE *node)
{
    ...
    if (node->type == ASTREE_CMD_BLOCK)
        output("\n");

    if (node->type != ASTREE_CMDL) {
    ...
    case ASTREE_IF:
        ...
        n2str(0, node->son[0]);
        output(") then ");
        if (node->son[1]->type == ASTREE_CMD_BLOCK)
            n2str(level, node->son[1]);
        else
            n2str(0, node->son[1]);
        break;
    case ASTREE_IF_ELSE:
        ...
        if (node->son[1]->type == ASTREE_CMD_BLOCK)
            ...
            n2str(0, node->son[1]);
        output(" else ");
        if (node->son[2]->type == ASTREE_CMD_BLOCK)
    ...
    case ASTREE_UNARY_MINUS:
        ...
        break;
    case ASTREE_POINTER:
        //fprintf(stderr, "EXP-UNARY-POINTER");
        output("*");
        n2str(0, node->son[0]);
        break;
    case ASTREE_DEREFERENCE:
