...
int main (void){
	...
	if ( matriz = LeArquivo ("grafo.txt", &n_vertices, matriz) ){
...
*/
	...
	if ((arq = fopen(nome_arquivo, "r")) == NULL){
		...
		printf("Características do grafo:\n");
		...
		*n_vertices = atoi(buffer); /* transforma o conteúdo de buffer em um inteiro */
		...
		printf("Vertices adjacentes (que contém uma aresta entre eles):\n");
		
		arestas = 0;
		while (1){ 
			memset(buffer,'\0',strlen(buffer)); /* zera buffer */
			fgets(buffer, sizeof(buffer), arq); /* pega a linha que o ponteiro arq esta apontando e coloca em "buffer" */
			if ( strcmp(buffer,"@\n") == 0 ){ /* se a condição do if for verdade buffer contém o caractere '@' */
				break; /* sai do while */
			}
			else{
