		...
		if ($_FILES['arquivo']['size'] < 2097152) //máximo de 2,0MB
		{
			...
			{	
				if(move_uploaded_file($_FILES['arquivo']['tmp_name'], $caminho)) 
				{
					...
					echo "<script>alert('Prova enviada com sucesso.');document.location='enviar_prova.php'</script>";
				}
				else
				{
					echo "<script>alert('Erro ao enviar a prova, entre em contato com o webmaster.');document.location='enviar_prova.php'</script>";
				}
			}	
			else
			{
				echo "<script>alert('A prova deve estar no formato zip ou rar.');document.location='enviar_prova.php'</script>";
			}
		}
		else
		{
			echo "<script>alert('Arquivo muito pesado. Máximo de 2.0MB.');document.location='enviar_prova.php'</script>";
		}
	}
	else
	{
		echo "<script>alert('Preencha todos os campos.');document.location='enviar_prova.php'</script>";
	}
...
<body>
	...
	<div style="width:800px; height:auto;">
		...
		<div style="width:600px; height:auto; margin-top:40px; margin-bottom:20px;">
			<p class="style2">Enviar Prova</p>
			<form action="enviar_prova.php" method="post" enctype="multipart/form-data" name="form1">
