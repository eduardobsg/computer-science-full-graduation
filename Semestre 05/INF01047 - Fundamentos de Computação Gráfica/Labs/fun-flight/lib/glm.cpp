...
glmFindGroup(GLMmodel* model, string name)
{
    ...
    while(group) {
        ...
        group = group->next;
        //if(!group)
        //   printf("\t\tHEY, YOU CRAZY???\n");
    }
    //printf("\t\tLooks like we've found it!\n");
    return group;
}
...
glmAddGroup(GLMmodel* model, string name)
{
    ...
    if (!group) {
        ...
        model->groups = group;
        model->numgroups++;
        //printf("New group %s added succesfully.\n",model->groups->name);
    }
...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    for (i = 0; i < nummaterials; i++) {
        ...
        model->materials[i].imageName = NULL;
        model->materials[i].modelTexture.imageData = NULL;
        model->materials[i].modelTexture.bpp = NULL;
        model->materials[i].modelTexture.width = NULL;
        model->materials[i].modelTexture.height = NULL;
        model->materials[i].modelTexture.texID = NULL;
        model->materials[i].modelTexture.type = NULL;
    }
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'N':
            ...
            break;
        case 'm':  //read image				//NEW CASE ADDED
			fgets(buf, sizeof(buf), file);
            sscanf(buf, "%s %s", buf, buf);
            model->materials[nummaterials].imageName = strdup(buf);
            //printf("matName=%s,",model->materials[nummaterials].name);
            //printf("nummaterials=%d,pathname=%s,imagename(filename)=%s\n\n,",nummaterials,model->pathname,model->materials[nummaterials].imageName);
...
glmFirstPass(GLMmodel* model, FILE* file) 
{
    ...
    unsigned    v, n, t;
    char        buf[128];
    char        num[MAX_GROUPS+1], groupName[128+MAX_GROUPS+1];
    int numGroup=0;
    memset(num,'\0',MAX_GROUPS+1);
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'v':               /* v, vn, vt */
            ...
            case 'u':
                /* eat up rest of line */
                fgets(buf, sizeof(buf), file);
                memset(groupName,'\0',MAX_GROUPS+1);
                sscanf(buf, "%s", groupName);
                num[numGroup]='i';
                num[numGroup+1]='\0';
                numGroup++;
                strncat (groupName, num, MAX_GROUPS+1);
                //printf("groupName=%s\n",groupName);
...
glmSecondPass(GLMmodel* model, FILE* file) 
{
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'v':               /* v, vn, vt */
            ...
            case 'u':
                fgets(buf, sizeof(buf), file);
                sscanf(buf, "%s %s", mat, mat);
                material = glmFindMaterial(model, mat);
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    //FILE *report;
	//report = fopen("debug.txt","w");
	//FILE *report1;
	//report1 = fopen("second.txt","w");
    group = model->groups;
    while (group) {
		//fprintf(report,"Drawing group %s.\n",group->name);
		//fprintf(report,"\tGroup's texture: %s.\n",model->materials[group->material].imageName);
		
		//printf("Drawing group %s.\n",group->name);
		//printf("\tGroup's texture: %s.\n",model->materials[group->material].imageName);
        if (mode & GLM_MATERIAL) {
            material = &model->materials[group->material];
            //glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, material->ambient);
            //glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, material->diffuse);
            //glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, material->specular);
            //glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, material->shininess); these are not in .mtl
            //glMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION, material->emmissive); these are not in .mtl

			//glBindTexture(GL_TEXTURE_2D, model->materials[group->material].modelTexture.texID);
        }
        if(model->materials[group->material].imageName != NULL)
		{
			//fprintf(report,"\tBinding texture %d.\n",model->materials[group->material].modelTexture.texID);
			//fprintf(report1,"\tname=%s,",model->materials[group->material].imageName);
			//fprintf(report1,"info=%d,end=%d\n",model->materials[group->material].modelTexture.texID,&(model->materials[group->material].modelTexture.texID));
			glBindTexture(GL_TEXTURE_2D, model->materials[group->material].modelTexture.texID);
		}
		else
		{
