...
bool LoadTGA(Texture * texture, char * filename)				// Load a TGA file
{
	...
	memset(arqName,'\0',100);
	strcpy(arqName, dir);
    strcat(arqName, filename);
    //printf("\tarqName=%s\n",arqName);
	fTGA = fopen(arqName, "rb");								// Open file for reading
...
bool LoadTGAPossible(Texture * texture, char * filename)				// Load a TGA file
{
	FILE * fTGA;												// File pointer to texture file
	...
	if(memcmp(uTGAcompare, &tgaheader, sizeof(tgaheader)) == 0)				// See if header matches the predefined header of 
	{																	// an Uncompressed TGA image
		return true;						// If so, jump to Uncompressed TGA loading code
	}
	else if(memcmp(cTGAcompare, &tgaheader, sizeof(tgaheader)) == 0)		// See if header matches the predefined header of
	{																// an RLE compressed TGA image
		return true;							// If so, jump to Compressed TGA loading code
	}
	else																	// If header matches neither type
	{
		//MessageBox(NULL, "TGA file be type 2 or type 10 ", "Invalid Image", MB_OK);	// Display an error
